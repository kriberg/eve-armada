...
from armada.eve.management.commands import start_task, end_task
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not len(args) == 1:
            raise CommandError("Please supply a output file for the generated models.")
        modelfile = args[0]
        ...
        if len(batch) > 0:
            ...
            print 'The SDE table is empty. Did you import the SDE dump correctly?'
            sys.exit(1)
        start_task('Fetching SDE table names')
        cursor.execute('select name from eve_sde_table where batch = %s',
                (batch,))
        self.eve_table_names = [r[0] for r in cursor.fetchall()]
        end_task()
        print '  Found %d tables in batch %d' % (len(self.eve_table_names),
                batch)
        ...
        start_task('Generating models')

        with open(modelfile, 'w') as f:
            for line in self.handle_inspection(options):
                f.write("%s\n" % line)
        end_task()
        start_task('Retrofitting primary keys to compund tables')
    ...
    def handle_inspection(self, options):
        ...
        yield "# It was made by armada's generateevemodels which has special"
        yield "# seasoning for importing the SDE dumps generated by zofu for postgresql."
        yield "# Don't later this file if you're not absoutely certain you should do so."
